...
(function () {
  ...
  /**
   * Add products here. Each product points to its own folder.
   * You can freely change displayName/subtitle without affecting asset paths.
   */
  const PRODUCTS = {
    tote: {
      key: 'tote',
      displayName: 'Cotton Tote Bag (BGCT1416-4oz)',
      subtitle: 'Upload your artwork and instantly preview it on our cotton tote bag',
      productDesc: '100% organic cotton, 12oz canvas. Print area: 10" × 12"',
      downloadName: 'tote-bag-mockup.png',
      assets: {
        base: 'products/tote/assets/base.png',
        displacement: 'products/tote/assets/displacement.png',
        mask: 'products/tote/assets/mask.png',
        shadow: 'products/tote/assets/shadow.png'
      }
    },
    socks: {
      key: 'socks',
      displayName: 'Custom Socks (ADD-SKU-HERE)',
      subtitle: 'Upload your artwork and instantly preview it on our socks',
      productDesc: 'Add your socks specs here (material, printable area, sizing).',
      downloadName: 'socks-mockup.png',
      assets: {
        base: 'products/socks/assets/base.png',
        displacement: 'products/socks/assets/displacement.png',
        mask: 'products/socks/assets/mask.png',
        shadow: 'products/socks/assets/shadow.png'
      }
    },
    tshirt: {
      key: 'tshirt',
      displayName: 'T-Shirt (ADD-SKU-HERE)',
      subtitle: 'Upload your artwork and instantly preview it on our t-shirt',
      productDesc: 'Add your tee specs here (DTG/DTF, printable area, sizing).',
      downloadName: 'tshirt-mockup.png',
      assets: {
        base: 'products/tshirt/assets/base.png',
        displacement: 'products/tshirt/assets/displacement.png',
        mask: 'products/tshirt/assets/mask.png',
        shadow: 'products/tshirt/assets/shadow.png'
      }
    }
  };
  ...
  const inquiryProductSelect = document.getElementById('inquiryProductSelect');
  ...
  async function loadAssetsForProduct(productKey) {
    const product = PRODUCTS[productKey];
    if (!product) throw new Error(`Unknown product key: ${productKey}`);

    state.assetsLoaded = false;

    const { base, displacement, mask, shadow } = product.assets;

    // tiny UI cue so switching feels intentional
  ...
  function applyProductCopy(productKey) {
    const product = PRODUCTS[productKey];
    if (!product) return;

    if (subtitleText) subtitleText.textContent = product.subtitle;
    if (productTitle) productTitle.textContent = product.displayName;
    if (productDesc) productDesc.textContent = product.productDesc;

    // keep inquiry dropdown synced
    if (inquiryProductSelect) inquiryProductSelect.value = productKey;
  }
  ...
  async function setProduct(productKey) {
    ...
    // sync header select (if changed programmatically)
    if (productSelect) productSelect.value = productKey;

    applyProductCopy(productKey);
    await loadAssetsForProduct(productKey);

    // If design already uploaded, re-render immediately on new base product
    if (state.designImg) render();
  }
  ...
  function render() {
    ...
    // Fit design to mask area, then apply user scale
    ...
    const intensity = state.dispIntensity * 0.3;
  ...
  document.getElementById('dispSlider').addEventListener('input', (e) => {
    state.dispIntensity = parseInt(e.target.value, 10);
  ...
  document.getElementById('downloadBtn').addEventListener('click', () => {
    ...
    const product = PRODUCTS[state.currentProductKey] || PRODUCTS.tote;

    const link = document.createElement('a');
    link.download = product.downloadName || 'mockup.png';
  ...
  (async function init() {
    ...
    if (productSelect) productSelect.value = state.currentProductKey;
    if (inquiryProductSelect) inquiryProductSelect.value = state.currentProductKey;

    applyProductCopy(state.currentProductKey);
    await loadAssetsForProduct(state.currentProductKey);
  })();
